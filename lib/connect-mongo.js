...
module.exports = function(connect) {
  ...
  function MongoStore(options, callback) {
    ...
    if (options.hasOwnProperty('stringify') ?
      ...
      this._serialize_session = function(cookie) {
        return cookie.toJSON();
      };
      this._unserialize_session = function(cookie) { return cookie; };
    }
          ...
          if (err) {
            ...
            if (session) {
              if (!session.expires || new Date < session.expires) {
                callback(null, self._unserialize_session(session.cookie));
              } else {
  ...
  MongoStore.prototype.set = function(sid, session, callback) {
    ...
    try {
      var s = {_id: sid, cookie: this._serialize_session(session.cookie)};
